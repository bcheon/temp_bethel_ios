...
│   ├── BKC/{App,Views,Services,Models,Resources}
│   ├── BKCNotificationServiceExtension/   # NSE — delivered 이벤트 기록
│   ├── BKCTests/                          # XCTest 유닛 테스트 (34개)
│   ├── BKCUITests/                        # XCUITest UI 테스트 (3개)
│   └── project.yml                        # XcodeGen 설정 (커밋. .xcodeproj 는 gitignore)
├── wordpress-plugin/bkc-push/       # PHP 8.1+ WP 플러그인
│   ├── bkc-push.php                       # 엔트리 + DB 마이그레이션
│   ├── includes/                          # 9개 클래스 (FCM, Dispatcher, REST API, ...)
│   ├── admin/{class-bkc-admin.php,views/} # 관리자 UI
│   ├── tests/                             # PHPUnit 8개 슈트 (41개)
│   ├── composer.json + composer.lock      # 둘 다 커밋 (재현 가능 빌드)
│   └── phpunit.xml.dist
├── well-known/apple-app-site-association  # Universal Links AASA (Team ID는 배포 시 교체)
├── .github/workflows/ci.yml         # GitHub Actions: lint + wp-test 매트릭스 + ios-test
├── fastlane/{Fastfile,Appfile}      # TestFlight 자동화 스켈레톤
...
```bash
make install      # brew + composer로 모든 도구 설치 (php, composer, xcodegen, act)
make test         # 사용 가능한 모든 테스트 (WP + iOS Mac에선)
make test-wp      # WP PHPUnit (어디서나 동작)
make test-ios     # iOS Xcode 시뮬레이터 (Mac 전용)
...
3. **AASA `appID`** — 현재 `YOUR_TEAM_ID.org.bkc.churchapp` placeholder. App Store 제출 전 실제 Apple Team ID로 치환 필수.
...
- **iOS UI 테스트는 `BKC_UITEST=1`** 환경변수로 Firebase 초기화 우회 (`BKCApp.swift` `isUITest` 가드). 이 가드 없이는 placeholder plist에서 `FIRApp.configure()` 크래시.

## CI 파이프라인 요약

`.github/workflows/ci.yml`:
- **lint-workflows** — actionlint
- **wp-test (matrix: PHP 8.1/8.2/8.3)** — composer install → phpunit
- **ios-test (macos-14)** — xcodegen → SPM resolve → `xcodebuild test` (iPhone 17 시뮬레이터)
...
- 새 PHPUnit 테스트 추가 시: `tests/Test_<Name>.php` (PascalCase basename) + `class Test_<Name> extends TestCase`. `phpunit.xml.dist` 자동 발견.
- 새 Swift 테스트 파일 추가: `BKCTests/<Name>Tests.swift` 만들면 xcodegen 재실행 시 자동 포함.
